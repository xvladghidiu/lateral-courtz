...
function VerticalLabel({ side, text, color }: { side: "left" | "right"; text: string; color: string }) {
  return (
    <div className={`fixed ${side}-0 top-0 bottom-0 z-[1] flex items-center pointer-events-none`}>
      <span className={`font-['Grize_Sports',sans-serif] text-[80px] uppercase tracking-[12px] ${color} [writing-mode:vertical-rl] ${side === "left" ? "rotate-180" : ""} select-none`}>
        {text}
      </span>
    </div>
  );
...
function NavLink({ dark }: { dark: boolean }) {
  const btnClass = dark
    ? "font-['Lixdu',sans-serif] uppercase tracking-[2px] px-5 py-2.5 text-[12px] text-[rgba(255,255,255,0.7)] bg-[rgba(255,255,255,0.1)] backdrop-blur-[16px] border border-[rgba(255,255,255,0.15)] rounded-xl shadow-[0_2px_10px_rgba(0,0,0,0.2)] hover:bg-[rgba(255,255,255,0.2)] hover:text-white transition-all no-underline"
    : "font-['Lixdu',sans-serif] uppercase tracking-[2px] px-5 py-2.5 text-[12px] text-[rgba(0,0,0,0.6)] bg-[rgba(255,255,255,0.8)] backdrop-blur-[16px] border border-[rgba(0,0,0,0.08)] rounded-xl shadow-[0_2px_10px_rgba(0,0,0,0.1)] hover:bg-[rgba(255,255,255,0.95)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.12)] transition-all no-underline";
